...
pub fn init_logger(framebuffer: &'static mut [u8], info: FrameBufferInfo) {
    LOGGER.init_once(move || LockedLogger::new(framebuffer, info));
}
...
impl Logger {
    ...
    fn newline(&mut self) {
        self.add_vspace(14 + LINE_SPACING);
    ...
    /// Puts the writer as the 0 X position.
...
macro_rules! fb_print {
    ($($arg:tt)*) => {
        $crate::fb_logger::_print(format_args!($($arg)*));
    };
}
